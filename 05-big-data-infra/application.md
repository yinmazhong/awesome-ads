...
实时大盘展示:
┌─────────────────────────────────────────┐
│  广告收入实时大盘  2024-01-15 14:30      │
│                                          │
│  今日收入: ¥12,345,678  (目标: ¥15,000,000)│
│  ████████████░░░░  82.3%                 │
│                                          │
│  收入趋势 (分钟级):                       │
│  ▁▂▃▄▅▆▇█▇▆▅▄▃▂▁▂▃▄▅▆▇                │
...
```
┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
│ 前端展示  │◄──│ 报表API  │◄──│ 查询引擎  │◄──│ 数据存储  │
│ (Web/App)│    │ 服务     │    │          │    │          │
└──────────┘    └──────────┘    └──────────┘    └──────────┘
                                     │
                          ┌──────────┼──────────┐
                          ▼          ▼          ▼
                    ClickHouse    Doris      Redis
                    (离线报表)   (多维分析)  (实时缓存)

优化手段:
  - 预聚合: 提前计算常用维度组合
  - 缓存: Redis 缓存热门查询结果
  - 物化视图: ClickHouse Materialized View
  - 异步查询: 大查询异步执行，结果推送
```
...
流量分配:
  - 用户级别分流 (保证同一用户始终在同一组)
  - Hash 分流: bucket = hash(user_id + experiment_id) % 100
  - 分层实验: 不同层的实验互不干扰

指标计算:
  - 核心指标: 收入、CTR、CVR、CPA
  - 护栏指标: 用户留存、负反馈率
  - 显著性检验: t-test / z-test / 贝叶斯
```
